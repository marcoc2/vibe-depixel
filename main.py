...
def run_kopf_lischinski_pipeline(img: Image.Image, img_array: np.ndarray, run_dir: str):
    ...
    print(
        f"Pontos de controle totais: {total_control_points} (média: {avg_control_points:.2f} por curva)"
    )
...
def run_deep_nn_pipeline(
    ...
    original_resized = np.kron(
        img_array, np.ones((upscale_factor, upscale_factor, 1), dtype=np.uint8)
    )
...
Examples:
  ...
  python main.py input/megaman.png --both
    ...
    parser.add_argument(
        "image_path", nargs="?", default=None, help="Path to the input pixel art image"
    )
    ...
    parser.add_argument(
        "--save-model", action="store_true", help="Save the trained Deep NN model"
    )
    ...
    parser.add_argument(
        "--lr-dir", help="Directory with low-resolution training images"
    )
    parser.add_argument(
        "--hr-dir", help="Directory with high-resolution training images"
    )
    parser.add_argument(
        "--val-lr-dir",
    ...
    parser.add_argument(
        "--sr", action="store_true", help="Run SR model inference on the input image"
    )
    parser.add_argument(
        "--scale",
        type=int,
        default=4,
        help="SR model upscale factor (1 for 1x enhancement, default: 4)",
    )
    parser.add_argument(
        "--checkpoint",
    ...
    parser.add_argument(
        "--resume", action="store_true", help="Resume training from last saved state"
    )
    parser.add_argument(
        "--no-perceptual",
        action="store_true",
    ...
    parser.add_argument(
        "--constant-lr",
        action="store_true",
        help="Disable CosineAnnealingLR scheduler — keep LR fixed throughout training",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0,
        metavar="N",
        help="Save a preview image every N epochs (0 = disabled)",
    )
    ...
    if args.train:
        ...
        sr_train(
            ...
            use_gan=args.gan,
            **({"gan_weight": args.gan_weight} if args.gan_weight is not None else {}),
